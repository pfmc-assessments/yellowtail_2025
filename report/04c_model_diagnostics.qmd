...
The maximum parameter gradient was `r signif(mod_out$maximum_gradient_component, 2)` and the jitter analysis showed that `r jit_good` out of `r jit_total` model runs with jittered initial values converged to the same model estimate. Exploratory MCMC runs indicated that posterior distributions largely matched these asymptotic distributions.
...
Model sensitivity to index data is in Figures [-@fig-index-sens-ssb] and [-@fig-index-sens-depl] and @tbl-sens-indices.
...
Sensitivity to modeling assumptions is in Figures [-@fig-model-sens-ssb] and [-@fig-model-sens-depl] and @tbl-sens-modeling.
...
Sensitivity to use and treatment of composition data is in Figures [-@fig-comp-sens-ssb] and [-@fig-comp-sens-depl] and @tbl-sens-modeling.
...
There is somewhat conflicting information about $M$ among data types and fleets (@fig-m-profile). The age and length data support a higher $M$ while the indices and recruitment penalties support a lower M. The likelihood contribution of the $M$ prior is small compared to the other data sources. The profile include a model with $M = 0.125$, which is similar to the median of the prior ($M = 0.126$). The difference in negative-log-likelihood between the estimated $M$ = `r mod_out$parameters["NatM_uniform_Fem_GP_1", "Value"] |> round(3)` and the $M = 0.125$ model was `r m_like_diff` units of log-likelihood compared to the prior likelihood contribution of the base model $M$ value of `r m_base` indicating signficant support in the data for the higher $M$ value.
