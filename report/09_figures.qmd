...
![Length-weight relationship with fitted data](figures/length_weight_with_data.png){#fig-length-weight}

![Maturity curve with fitted data.](figures/maturity_with_data.png){#fig-maturity}

## Model

### Bridging

![Spawning output (trillions of eggs, top), and relative spawning output (bottom) from first steps in model bridging. Catches from recreational fleets are combined when catch is updated. Uncertainty is only shown for the 2017 model.](figures/bridging/bridging1_comparison.png){#fig-bridge1}

![Age 4+ biomass (mt) from first steps in model bridging. Catches from recreational fleets are combined when catch is updated.](figures/bridging/bridging1compare18_smrybio.png){#fig-bridge1-smry-bio}
...
![Age 4+ biomass (mt) from second steps (updating composition data) in model bridging. "Exp" is expanded PacFIN data. Composition data from recreational fleets are combined as length and age data are renanalyzed. Every model run includes two rounds of Francis tuning of composition data. Bias adjustment ramp is updated when data are extended.](figures/bridging/bridging2compare18_smrybio.png){#fig-bridge2-smry-bio}

![Spawning output (trillions of eggs, top), and relative spawning output (bottom) from third steps (model changes) in model bridging. Uncertainty is shown for the 2017 model and the final bridging step (the 2025 base model).](figures/bridging/bridging3_comparison.png){#fig-bridge3}

![Age 4+ biomass (mt) from third steps (model changes) in model bridging..](figures/bridging/bridging3compare18_smrybio.png){#fig-bridge3-smry-bio}
...
![Spawning output (trillions of eggs, top), and relative spawning output (bottom) for index sensitivities.](figures/sensitivities/indices_comparison.png){#fig-indices-comparison}

![Spawning output (trillions of eggs, top), and relative spawning output (bottom) for modeling sensitivities.](figures/sensitivities/modeling_comparison.png){#fig-modeling-comparison}

![Spawning output (trillions of eggs, top), and relative spawning output (bottom) for composition data sensitivities](figures/sensitivities/comp_data_comparison.png){#fig-comp_data-comparison}
