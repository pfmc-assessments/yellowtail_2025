...
<!-- copied text from 2017, may want to try to slim down (though it is not as excessive as some assessments) -->
...
-   Common documentation of data streams and sources to support fishery independent and fishery dependent indices and compositional data could reduce the burden on assessment analysts to provide details about each data source, and allow reviewers a robust source of information on the most important, common data sources for any given stock assessment cycle.
    -   The stock assessment program at \gls{nwfsc} has made great strides in improving data processing since 2017 through a number of standardized R packages for commercial and survey data.
...
-   Foreign trawl catches from 1966-1976 have been added to the commercial fleet.
...
Due to the richness of age composition data, most life history parameters were estimable. Natural mortality and growth are both estimated separately for males and females with shared parameters for length at age 2 and CV of length-at-age for young fish. Separate values for small fish were not estimable, nor was the four-parameter Richards growth curve. Male values were estimated as an offset of female values. A prior was placed on female natural mortality, but the data overwhelmed it (CITE PROFILE). Unfished recruitment is estimated. Steepness of the stock-recruit relationship is fixed at `r mod_in$ctl$SR_parms['SR_BH_steep', 'INIT']` per the PFMC terms of reference for rockfish assessments. The standard deviation of log recruitment deviations is `r mod_in$ctl$SR_parms['SR_sigmaR', 'INIT']`, the same value used in 2017. Reevaluating the tuning of this parameter indicated no changes were necessary [@methot_adjusting_2011]. Recruitment deviations during the "main" period (from `r mod_in$ctl$MainRdevYrFirst` to `r mod_in$ctl$MainRdevYrLast`) were forced to sum to zero, though sensitivity to this was examined.

Length, age, and age-at-length composition data weights were tuned using the Francis method [@francis_data_2011]. An additional standard error was estimated for the \gls{s-tri} and combined hook and line surveys. The former is an older survey with unrealistic inter-annual variability for a rockfish. The latter is a newer survey that combines hook and line data from two state sampling programs. While attempts were made to control for differences between the states, it is possible survey effort was not perfectly standardized. No additional standard error was estimated for the \gls{s-wcgbt}.

All selectivity was assumed to be length-based and used a double-normal functional form. We explored age-based selectivity, but due to the sexual dimorphism of Yellowtail rockfish, fitting the composition data well would have required a more complex formulation with additional estimated parameters. Selectivity was estimated to be sex-specific and dome-shaped for the recreational fleet and the hook and line survey, with three base parameters (X) and two sex specific parameters (descending limb and X) being estimated. All other fleets had a single shared asymptotic selectivity for both sexes. Early explorations into dome-shaped selectivity for fleets using predominantly trawl gear indicated that few fish were actually observed at the lengths of the estimates for the descending limbs. Sex-specific selectivity for fleets using trawl gear did not visibly improve fits to composition data, so were not considered for the base model.

Selectivity was constant across time for all but the recreational and at-sea hake fleets. Recreational selectivity is estimated across three time periods: start to `r mod_in$ctl$Block_Design[[1]][1]-1`, `r mod_in$ctl$Block_Design[[1]][1]` to `r mod_in$ctl$Block_Design[[1]][2]` when depth restrictions were in place to prevent catch of overfished rockfish, and `r mod_in$ctl$Block_Design[[1]][3]` to `r mod_in$ctl$Block_Design[[1]][4]` to account for the growing longleader fleet in Oregon and avoidance of Black rockfish in Washington, both of which led to recreational trips occurring farther offshore and catching larger fish. The sex-specific paramters were assumed to be constant. The at-sea hake fleet estimates selectivity across two time periods: the start to `r mod_in$ctl$Block_Design[[2]][1]-1` and `r mod_in$ctl$Block_Design[[2]][1]` to `r mod_in$ctl$Block_Design[[2]][2]`. While no particular management change warranted this block, the at-sea hake fleet is highly mobile from year to year and does not target Yellowtail rockfish, so changes in selectivity across time are likely. Length compositions became smaller than expected around the time of the block, particularly for females, a misfit that was not seen across other fleets. The hake fishery has occurred farther south in recent years, where Yellowtail do tend to be slightly smaller at age (CITE FIGURE).
